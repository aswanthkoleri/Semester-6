...
def avg_entropy_w_partitions(data, splitting_att, target_attribute):
    ...
    for partition_key in partitions.keys():
        ...
        splitGain +=-(partition_n/n)*math.log(partition_n/n,2)
    return avg_ent, partitions,splitGain
...
def id3(data, uniqs, remaining_atts, target_attribute):
    ...
    for remaining_att in remaining_atts:
        avg_ent, partitions,splitGain = avg_entropy_w_partitions(data, remaining_att, target_attribute)
        info_gain = (ent - avg_ent)/splitGain
...
def main():
    data = load_csv_to_header_data('resources/dataset.csv')
    data = project_columns(data, ['Buying','Maint','Doors','Persons','LugBoot','Safety','Output'])
